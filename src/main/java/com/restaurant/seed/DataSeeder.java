...
public class DataSeeder {
    ...
    private List<MenuItem> seedMenuItems(EntityManager em, List<Menu> menus) {
        ...
        for (Menu menu : menus) {
            for (String itemName : foodItems) {
                ...
                mi.setName(itemName);
                mi.setDescription("Recipe for " + itemName);
    ...
    private List<User> seedUsers(EntityManager em) {
        ...
        manager.setPasswordHash(BCrypt.hashpw("123", BCrypt.gensalt()));
        ...
        User waitStaff = new User();
        waitStaff.setName("Wait Staff");
        waitStaff.setUsername("waitstaff");
        waitStaff.setPasswordHash(BCrypt.hashpw("123", BCrypt.gensalt()));
        waitStaff.setRole(UserRole.WAIT_STAFF);
        waitStaff.setEmail("waitstaff@example.com");
        waitStaff.setActive(true);
        em.persist(waitStaff);
        users.add(waitStaff);

        User cook = new User();
        cook.setName("Cook");
        cook.setUsername("cook");
        cook.setPasswordHash(BCrypt.hashpw("123", BCrypt.gensalt()));
        cook.setRole(UserRole.COOK);
        cook.setEmail("cook@example.com");
        cook.setActive(true);
        em.persist(cook);
        users.add(cook);
        ...
        shipper.setPasswordHash(BCrypt.hashpw("123", BCrypt.gensalt()));
